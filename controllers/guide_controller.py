...
def create_guide_category(category: GuideCategoryCreate, db: Session = Depends(get_db)):
...
def read_guide_categories(skip: int = Query(0, alias="pagina", ge=0), limit: int = Query(5, alias="por_pagina", ge=1), db: Session = Depends(get_db)):
...
def read_guide_category(category_id: int, db: Session = Depends(get_db)):
...
def update_guide_category(category_id: int, category: GuideCategoryCreate, db: Session = Depends(get_db)):
...
@router.delete("/guide-categories/{category_id}")
def delete_guide_category(category_id: int, db: Session = Depends(get_db), tags=["guide"]):
    db_category = db.query(GuideCategory).filter(GuideCategory.id == category_id).first()
    if db_category is None:
        raise HTTPException(status_code=404, detail="Category not found")
...
def read_guides(skip: int = Query(0, alias="pagina", ge=0), limit: int = Query(5, alias="por_pagina", ge=1), db: Session = Depends(get_db)):
...
def update_guide(guide_id: int, guide: GuideCreate, db: Session = Depends(get_db)):
